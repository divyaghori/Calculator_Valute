...
public abstract class HidedDatabase extends RoomDatabase {
    ...
    public static HidedDatabase getDatabse(Context context)
    {
        ...
        Util util=new Util();
        database= Room.databaseBuilder(context, HidedDatabase.class, util.getFolder(context)+"/hideFiles").allowMainThreadQueries().fallbackToDestructiveMigration().build();
        return database;
    }
